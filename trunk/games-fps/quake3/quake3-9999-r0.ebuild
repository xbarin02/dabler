...
IUSE="dedicated opengl teamarena +openal curl vorbis voice mumble -zlib"
...
src_compile() {
	...
	notbuildit() { use $1 && echo 0 || echo 1 ; }
	...
	emake \
		...
		USE_INTERNAL_ZLIB=$(notbuildit zlib) \
		USE_LOCAL_HEADERS=0 \
...
src_install() {
	...
	if use opengl ; then
		GLR=$(echo renderer_opengl*)
		dogameslib.so "${GLR}"
		dosym "$(games_get_libdir)/${GLR}" "${GAMES_DATADIR}/${PN}/${GLR}"
	fi
